...
class AirCargoProblem(Problem):
            ...
            for start in self.airports:
                for plane in self.planes:
                    for cargo in self.cargos:
                        precond_pos = [expr('At({}, {})'.format(cargo, start)), expr('At({}, {})'.format(plane, start))]
                        precond_neg = []
                        effect_add = [expr('In({}, {})'.format(cargo, plane))]
                        effect_rem = [expr('At({}, {})'.format(cargo, start))]

                        load = Action(expr('Load({}, {}, {})'.format(cargo, plane, start)),
                ...
                for plane in self.planes:
                    for cargo in self.cargos:
                        precond_pos = [expr('In({}, {})'.format(cargo, plane)),
                                       expr('At({}, {})'.format(plane, destination))]
                        precond_neg = []
                        effect_add = [expr('At({}, {})'.format(cargo, destination))]
                        effect_rem = [expr('In({}, {})'.format(cargo, plane))]

                        unload = Action(expr('Unload({}, {}, {})'.format(cargo, plane, destination)),
                                        [precond_pos, precond_neg],
                                        [effect_add, effect_rem])
    ...
    def result(self, state: str, action: Action):
        ...
        # (positive or negated atomic sentences). The precondition defines the states from which the action can be
        # executed, and the effect defines the result of executing the action. So, for each fluent in the old state,
        # we need to look in the action's effects  to determine how the fluent will be carried over to the new state.
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        kb = PropKB()
        kb.tell(decode_state(node.state, self.state_map).pos_sentence())
        return sum([1 for subgoal in self.goal if subgoal not in kb.clauses])
