...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Add a set of PgNode_a to a_levels
        self.a_levels.append(set())
        # List of sets of PgNode_s
        s_level = self.s_levels[level]
        # Iterate through all possible actions for the problem
        for a in self.all_actions:
            # Add a PgNode_a to a_levels[level]
            a_node = PgNode_a(a)
            ...
            for s_node in s_level:
                ...
                # Connect added action node to the S node instances in the appropriate s_level set.
    ...
    def add_literal_level(self, level):
        ...
        # Add a set of PgNode_s to s_levels
        self.s_levels.append(set())
        # Previous level of a_levels
        a_level = self.a_levels[level-1]
        # Iterate through all a_nodes in a_level
        for a_node in a_level:
            # Add new literal nodes/ s_nodes which are the effect of previous level a_nodes to the level
            # Create connections between s-child and a-parent nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return bool(set(node_a1.action.effect_add).intersection(node_a2.action.effect_rem) or\
                    set(node_a2.action.effect_add).intersection(node_a1.action.effect_rem))
        '''
        return bool( not set(node_a1.action.effect_add).isdisjoint(set(node_a2.action.effect_rem)) or \
                     not set(node_a2.action.effect_rem).isdisjoint(set(node_a1.action.effect_add)))
        '''
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return any(e in node_a1.action.effect_rem for e in node_a2.action.precond_pos) or \
               ...
               any(e in node_a2.action.effect_add for e in node_a1.action.precond_neg)
    ...
    def h_levelsum(self) -> int:
        ...
        def goal_cost(goal_state):
            for level, s_nodes in enumerate(self.s_levels):
                if PgNode_s(goal_state, True) in s_nodes:
                    # Stopping at the first level at which appears in the graph
                    return level
            return 0

        return sum(goal_cost(g) for g in self.problem.goal)
